...
**新增文件**：
- `TransferProgressDTO.java` - 单个调拨进度响应DTO
- `BatchTransferProgressDTO.java` - 批量调拨进度响应DTO
- `TransferProgressService.java` - 进度计算服务
- `TransferProgressController.java` - 进度查询接口
...
**新增文件**：
- `TransferNotificationService.java` - 调拨通知服务

**核心功能**：
- 调拨发起时发送站内信通知(仅用户请求调拨)
- 调拨到达时发送站内信通知(仅用户请求调拨)
- 库存平衡调拨不发送通知(系统内部操作)
- 使用现有`tb_notification`表存储通知

**通知规则**：
- 用户请求调拨(USER_REQUEST): 发送通知给目标用户
- 库存平衡调拨(INVENTORY_BALANCE): 不发送通知

### 3. 新增延迟检测定时任务

**新增文件**：
- `TransferDelayCheckTask.java` - 调拨延迟检测定时任务

**核心功能**：
- 每小时检查一次超时调拨
- 查询所有状态为IN_TRANSIT且超过预计到达时间的调拨记录
- 控制台打印告警日志,包含详细信息
- Cron表达式: `0 0 * * * ?`
...
**修改文件**：
- `TransferService.java` - 注入TransferNotificationService
...
## 2026年3月9日 - Phase A一致性闭环增强

**改进人**：GPT-5.2(Cusor Coding Agent)

### 1. 调拨时间字段统一
- `TransferService.createTransfer`统一写入`estimatedArrivalTime`，不再将`completeTime`当作预计到达
- `TransferService.completeTransfer`写入`actualArrivalTime`并同步`completeTime`
- 保证`TransferDelayCheckTask`与`TransferProgressService`的进度/延迟计算可用

### 2. 预留字段补齐与超期释放闭环
- `BookReturnService`就地预留创建`BookBorrow(RESERVED)`时补齐:
  - `reservedTime/pickupDeadline/pickupLibraryId/dueTime`
- `TransferService.completeTransfer`创建/更新`BookBorrow(RESERVED)`时补齐`dueTime`
- `ReservationExpireTask`增强:
  - 超期释放时回滚用户`current_borrow_count`
  - 扣信用分并必要时冻结账号
  - 释放副本后自动兑现下一位预约者
- 新增`ReservationFulfillmentService`用于复用“兑现下一预约者”逻辑

### 3. 用户调拨列表可查询
- `BookTransfer`新增字段`receiverUserId`(对应数据库字段`receiver_user_id`)
- 创建用户请求调拨时写入接收用户ID(借书调拨/预约兑现调拨)
- `TransferProgressService`实现用户调拨分页查询
- `TransferProgressController`开放`GET /api/transfer/my-transfers`真实返回数据


## 2026年3月9日 - 调拨完成接口优化与定时任务测试功能

### 改动概述

1. 优化调拨完成接口，移除不必要的时间参数
2. 新增定时任务手动触发接口，提高测试效率
...
**改动文件**：
- `TransferController.java`：移除时间参数，简化请求DTO
- `TransferService.java`：移除时间参数，自动使用当前时间

**改动内容**：
- `/api/transfer/complete` 接口移除 `actualArriveTime` 参数
- `/api/transfer/complete-batch` 接口移除 `actualArriveTime` 参数
...
**新增接口**：
- `POST /api/task/trigger/reservation-expire`：触发预约超期释放任务
- `POST /api/task/trigger/transfer-delay-check`：触发调拨延迟检查任务
- `POST /api/task/trigger/inventory-balance`：触发库存平衡任务
- `POST /api/task/trigger/reservation-warning`：触发预约提醒任务
...
**测试步骤**：
1. 准备IN_TRANSIT状态的调拨记录
2. 调用 `/api/transfer/complete` 接口（不传时间参数）
3. 验证数据库时间字段自动设置
...
**测试步骤**：
1. 准备超期数据
2. 调用 `/api/task/trigger/reservation-expire` 接口
3. 验证超期释放逻辑正确执行
